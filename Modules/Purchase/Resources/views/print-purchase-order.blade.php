...
<html lang="fr">
    <head>
        ...
        <title>{{ __('Bon de commande fournisseur') }} - {{ $purchase->reference }} | {{ current_company()->name }}</title>
        <!-- Favicon -->
    ...
    <body>
        {{-- Header --}}
        {{-- @if($purchase->logo)
            <img src="{{ $purchase->getLogo() }}" alt="logo" height="100">
        @endif --}}
        {{-- <div class="logo">
                ...
                <tr>
                    <td class="border-0 pl-0" width="70%">
                        ...
                        @if(current_company()->address)
                            <p class="company-address">
                                {{ __('Adresse de livraison') }}: {{ current_company()->address }}
                            </p>
                        @endif
                    ...
                    <td class="border-0 pl-0" width="60%">
                        <h4 class="text-uppercase cool-gray">
                            <strong>{{ __('Commande') }} {{ $purchase->reference }}</strong>
                        </h4>
                        <p><b>{{ __('Date de la commande') }}</b>: <strong>{{ \Carbon\Carbon::parse($purchase->date)->format('d/m/Y') }}</strong></p>
                        <p><b>{{ __("Responsable achat") }}</b>: {{ $buyer->user->name }}</p>
                    </td>
                </tr>
            </tbody>
        </table>
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <table class="table table-items">
                    <thead>
                        <tr>
                            ...
                            @if($purchase->product_discount_percentage)
                    ...
                    <tbody>
                        ...
                        <tr style="border-bottom: 1px solid #dee2e6;">
                            ...
                            @if($purchase->product_discount_amount)
                                <td class="text-right">
                        ...
                        @if($purchase->discount_amount > 0)
                            <tr>
                                <td colspan="{{ $purchase->table_columns - 2 }}" class="border-0"></td>
                                <td colspan="{{ $purchase->table_columns - 2 }}" class="border-0"></td>
                                <td colspan="{{ $purchase->table_columns - 2 }}" class="border-0"></td>
                                <td class="text-right pl-0">{{ __('Réduction') }} ({{ $purchase->discount_percentage }})%</td>
                                <td class="text-right pr-0">
                        ...
                        @if($purchase->tax_amount > 0)
                            <tr>
                                ...
                                <td colspan="{{ $purchase->table_columns - 2 }}" class="border-0"></td>
                                <td colspan="{{ $purchase->table_columns - 2 }}" class="border-0"></td>
                                <td class="text-right pl-0">{{ __('Taxe') }}  ({{ $purchase->tax_percentage }}%)</td>
                                <td class="text-right pr-0">
                        ...
                        @if($purchase->tax_rate)
                            <tr>
                                ...
                                <td colspan="{{ $purchase->table_columns - 2 }}" class="border-0"></td>
                                <td colspan="{{ $purchase->table_columns - 2 }}" class="border-0"></td>
                                <td class="text-right pl-0">{{ __('purchases::purchase.tax_rate') }}</td>
                                <td class="text-right pr-0">
                        ...
                        @if($purchase->shipping_amount > 0)
                            <tr>
                                ...
                                <td class="text-right pr-0">
                                    {{ format_currency($purchase->shipping_amount) }}
                                </td>
                            </tr>
                        @endif
                            <tr>
                ...
                @if($purchase->term)
                    <p>
                        {{ __('Notes') }}: {!! $purchase->term !!}
                    </p>
                @endif

                {{-- <p>
                    {{ trans('purchases::purchase.amount_in_words') }}:
                </p> --}}
                <p>
                    Veuillez utiliser cette reference pour votre paiement: {{ $purchase->reference }}
                </p>
                <p>
                    Modalité de paiement: {{ $purchase->payment_term }}
                </p>
                @if($purchase->shipping_date)
                <p>
                    Date de livraison prévue: {{ \Carbon\Carbon::parse($purchase->shipping_date)->format('d/m/Y') }}
                </p>
                @endif
