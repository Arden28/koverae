                  ...
                  <div class="card">
                    <div class="k_control_panel_main d-flex flex-wrap flex-grow-2 flex-nowrap justify-content-between align-items-start gap-5">
                        ...
                        <div class="k_control_panel_breadcumbs d-flex align-items-center gap-1 order-0 h-lg-100">
                            <a wire:click="$set('addIndustry', true)" class="btn btn-outline-primary k_form_button_create">
                                {{ __('Nouveau') }}
                            </a>
                        ...
                        <div class="k_control_panel_navigation d-flex flex-wrap align-items-center justify-content-end gap-l-1 gap-xl-5 order-1 order-lg-2 flex-grow-1">
                            ...
                            <nav class="k_pager align-items-center d-flex gap-2">
                                ...
                                <span class="btn-group d-print-none">
                  ...
                  <table class="table card-table table-vcenter text-nowrap datatable">
                    <thead>
                      <tr>
                        ...
                        <th><button class="table-sort" data-sort="sort-name">{{ __("Titre") }}</button></th>
                        <th><button class="table-sort" data-sort="sort-name">{{ __('Abréviation') }}</button></th>
                        <th></th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            <td><input wire:model.blur="selectedTitles" value="{{ $i->id }}" class="form-check-input m-0 align-middle" type="checkbox" aria-label="Select invoice"></td>
                            <td>
                                {{ $i->name }}
                            </td>
                            <td>
                            ...
                            <td class="text-end">
                                ...
                                <div class="dropdown-menu dropdown-menu-end">
                                    <a href="#" wire:click="selectedId({{ $i->id }})" data-bs-toggle="modal" data-bs-target="#salesTeam-{{ $i->id }}" class="dropdown-item text-danger">
                                        {{ __('Supprimer') }}
                                    </a>
                                </div>
                                </span>
                        ...
                        <!-- Delete Modal -->
                        <div wire:ignore class="modal modal-blur fade" id="salesTeam-{{ $i->id }}" tabindex="-1" role="dialog" aria-hidden="true">
                            <div class="modal-dialog modal-sm modal-dialog-centered" role="document">
                            <div class="modal-content">
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                <div class="modal-status bg-danger"></div>
                                ...
                                <!-- Download SVG icon from http://tabler-icons.io/i/alert-triangle -->
                                <svg xmlns="http://www.w3.org/2000/svg" class="icon mb-2 text-danger icon-lg" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M12 9v2m0 4v.01" /><path d="M5 19h14a2 2 0 0 0 1.84 -2.75l-7.1 -12.25a2 2 0 0 0 -3.5 0l-7.1 12.25a2 2 0 0 0 1.75 2.75" /></svg>
                                <h3>{{ __("Êtes vous sûr de vouloir supprimer le devis ". $i->reference.' ?') }}</h3>
                                ...
                                <div class="w-100">
                                    ...
                                    <div class="col"><a wire:click.prevent="delete({{ $i->id }})" class="btn btn-danger w-100" data-bs-dismiss="modal">
                                        {{ __('Supprimer') }}
                                        </a></div>
                                    </div>
                                </div>
                        ...
                        <!-- Add Title -->
                        @if($addIndustry)
                        <form wire:submit="storeIndustry()" wire:transition>
                            @csrf
                            <tr>
                                <td><input wire:model="selectedTitles" class="form-check-input m-0 align-middle" type="checkbox" aria-label="Select invoice"></td>
                                <td>
                                    <input type="text" class="k_input" wire:model="name">
                                </td>
                                <td>
                                    <input type="text" class="k_input" wire:model="full_name">
                                </td>
                                <td>
                                    <button type="submit"wire:click="storeIndustry()">{{ __('Sauvegrader') }}</button>
                                </td>
                            </tr>
                        </form>
                        @endif
